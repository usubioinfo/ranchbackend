...
const runBlast = (id, filedata, program, genome, word, target, evalue) => {
    ...
    const child = require('child_process').execSync(`${program} -db ${path.join(__dirname, "../data/"+genome)} -query ${filedata} -max_target_seqs ${target} -word_size ${word} -evalue ${evalue} -num_threads 20 -outfmt 6 -out ${path.join(__dirname,`preddata/pred${id}.out.xml`)}`)
